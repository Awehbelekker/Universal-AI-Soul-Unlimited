...
class AIProviderManager:
    ...
    def register_provider(self, provider_id: str, provider: Any, 
        ...
        logger.info(f"Registered provider: {provider_id} ({provider_type.value}, priority: {priority.name})")
    
    async def generate(self, prompt: str, provider_id: Optional[str] = None, **kwargs) -> str:
        """Generate response using specified or optimal provider"""
        if provider_id and provider_id in self.providers:
            return await self._generate_with_provider(provider_id, prompt, **kwargs)
        ...
        try:
            return await self._generate_with_provider(selected, prompt, **kwargs)
        except Exception as e:
            logger.warning(f"Provider {selected} failed: {e}")
            return await self._fallback_generate(prompt, exclude=[selected], **kwargs)
    
    def _select_provider(self) -> Optional[str]:
        """Select best available provider"""
        active = [(pid, p) for pid, p in self.providers.items() if p['active']]
        if not active:
            return None
    ...
    async def _generate_with_provider(self, provider_id: str, prompt: str, **kwargs) -> str:
    ...
    async def _fallback_generate(self, prompt: str, exclude: List[str], **kwargs) -> str:
        ...
        for pid, pinfo in self.providers.items():
            if pid not in exclude and pinfo['active']:
                try:
                    ...
                    return await self._generate_with_provider(pid, prompt, **kwargs)
                except Exception as e:
