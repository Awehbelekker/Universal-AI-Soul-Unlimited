...
class HybridModelManager:
    ...
    def __init__(self, models_dir: str = "models"):
        ...
        logger.info(f"Default model: qwen2.5-3b (LITE tier)")
    ...
    def get_recommended_model(self, device_info: Dict) -> str:
        ...
        if ram_mb >= 6144 and storage_mb >= 6000:
            ...
            if gpt_oss['status'] == ModelDownloadStatus.DOWNLOADED:
                logger.info("📱 Recommending: gpt-oss-20b (PRO - already downloaded)")
                return 'gpt-oss-20b'
            
            # Suggest download if on WiFi
            if on_wifi:
                logger.info("📱 Recommending: gpt-oss-20b (PRO - download available)")
                return 'gpt-oss-20b'
    ...
    async def download_model(
        ...
        try:
            ...
            for mb_downloaded in range(0, total_mb, 100):
                await asyncio.sleep(0.1)  # Simulate download chunk
    ...
    def check_download_requirements(self, model_id: str) -> Dict:
        ...
        warnings.append("Download on WiFi recommended (large file)")
        
        return {
            'can_download': len([w for w in warnings if 'Insufficient' in w]) == 0,
            'warnings': warnings,
        ...
        try:
            ...
            for model_id, model_status in status.items():
                if model_id in self.models:
                    status_value = model_status.get('status', 'not_downloaded')
                    self.models[model_id]['status'] = ModelDownloadStatus(status_value)
    ...
    async def infer(
        ...
        if model['status'] != ModelDownloadStatus.DOWNLOADED:
            return {
                ...
                'can_download': model['status'] == ModelDownloadStatus.NOT_DOWNLOADED
            }
